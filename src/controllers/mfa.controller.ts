...
export const verifyAndEnableMFA = async (req: Request, res: Response) => {
  ...
  const mfaConfig = await UserMfaConfig.findOne({ where: { user_id: userId } });
  if (!mfaConfig || !mfaConfig.dataValues.secret)
    return res
      .status(404)
      .json({
        message:
          "MFA setup not found. Please run the setup (QR code) step first.",
      });
...
export const resetMFA = async (req: Request, res: Response) => {
  try {
    const userId = Number(req.token.id);
    if (!userId) return res.status(400).json({ message: "user not found !!" });

    //check the user-mfa config exist or not:
    const isActive = await UserMfaConfig.findOne({where:{user_id:userId, is_active: true}});
    if(!isActive) return res.json({message:"mfa already deactive !!"})
    ...
    return res.status(200).json({message:"deactive MFA setup successfully !!"})
  } catch (error) {
    console.log(error);
